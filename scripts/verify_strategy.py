...
def generate_test_data():
    ...
    for i in range(5):
        data.append(
            {"Open": 100 + i, "High": 105, "Low": 100, "Close": 101 + i, "Volume": 1000}
        )
    ...
    for i in range(len(df)):
        ...
        if signal and signal["signal_type"] == "entry":
            ...
            if abs(signal["take_profit"] - expected_tp) < 0.001:
                ...
                print(
                    f"FAILURE: Expected TP {expected_tp}, got {signal['take_profit']}"
                )
            return
...
def test_short_tp_calculation():
    ...
    for i in range(5):
        data.append(
            {"Open": 105 - i, "High": 105, "Low": 100, "Close": 104 - i, "Volume": 1000}
        )
